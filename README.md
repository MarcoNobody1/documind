...
| Layer | Technology |
| --- | --- |
| Frontend | Angular (standalone components, SCSS, SSE streaming chat, client-side routing + auth guard) |
| Backend | ASP.NET Core on .NET 10 (C# 14), Clean Architecture |
| Auth | ASP.NET Core Identity — cookie auth + CSRF, no route requires it yet (see Key decisions) |
