...
- **Auth screens restyled on Helm primitives (ui-redesign, auth screens slice)** — `/login` and `/register` had rendered functionally unusable since PR1's Tailwind preflight landed: `input`/`button` carried `border: 0`, a transparent background, and zero padding — there was no visible way to tell where to type — and the `h2` heading was visually identical to body text. Both templates are rebuilt on `hlm-card` and `hlmBtn` (already in the repo from PR3) plus two Helm primitives pulled by this slice, `hlmInput` and `hlmLabel`. The dead `.auth`/`.warning` class names — matching no rule anywhere in the document, confirmed by walking `document.styleSheets` — are gone. `Login` and `Register` each declare their host display (`flex flex-1 items-center justify-center p-6`, per ADR-L) because the shared `<main>` (PR3) is now a full-viewport flex column with no centring of its own; the auth screens own their own centring rather than assuming one that no longer exists. Pulling `hlmInput` is the first thing in this client to import `@angular/forms` (through `@spartan-ng/brain/field`'s accessible `describedby` wiring), which is why the initial bundle crosses the 500 kB soft budget for the first time — 501.16 kB raw / 124.57 kB estimated transfer, up from PR3's 453.88 kB/115.73 kB — a non-blocking build warning, comfortably under the 1 MB hard error budget. No auth logic changed: `Login`/`Register` still call `AuthService.login`/`.register` exactly as before; only the templates and their host classes moved. This slice was promoted ahead of the conversation restyle after the login page was found broken rather than merely unstyled (see Known follow-ups); the conversation pane, message list, and citation chips are untouched and remain the next slice.
