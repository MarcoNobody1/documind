...
- **Cancelación y motivos de rechazo de subida honestos (ui-redesign, PR2 de 5)** — dos correcciones de comportamiento, sin restilizado visual. `ChatService.ask()` crea ahora un `AbortController` por petición y expone `stop()`. Un nuevo campo `outcome?: 'complete' | 'stopped' | 'failed'` en `ChatMessage` registra cómo terminó cada turno; se fija únicamente a través del asistente `patchAssistantMessage` ya existente en el servicio, nunca en el turno del usuario, así que el contrato previo de diff cero sobre `chat.service.spec.ts` se mantiene intacto (verificado contra `main`, no asumido) — las pruebas de cancelación viven en un `chat.service.abort.spec.ts` nuevo. La detección de cancelación lee `controller.signal.aborted` en lugar de `error.name === 'AbortError'`, porque un rechazo que surge del lector SSE no tiene garantizado llevar ese nombre en todas las implementaciones de fetch ni en jsdom. Detener a mitad de streaming conserva la respuesta parcial, limpia `isStreaming` a través del mismo `finally` que ya se ejecutaba al completar y al fallar, y ya no relanza la excepción hacia el `catch` de `Chat.ask()` — que antes repintaba una detención deliberada como "Failed to get an answer. Is the API running?". Un controlador obsoleto de una petición ya terminada nunca puede cancelar una posterior: el `finally` de `ask()` limpia el campo, protegido por identidad para que una petición que tarda en terminar no pueda limpiar el controlador de una petición más nueva. Por separado, `Upload` ahora muestra el motivo real de rechazo de la API (PDF inválido, archivo vacío, archivo demasiado grande) a través de una nueva función pura `uploadErrorMessage()`, reservando el mensaje genérico "Is the API running?" para el único caso que realmente describe: no se recibió ninguna respuesta (`status === 0`).
